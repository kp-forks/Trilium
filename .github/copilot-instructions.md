...
7. **Documentation edits have rules** - The Script API reference is auto-generated by `apps/build-docs` into the gitignored `site/` (published to docs.triliumnotes.org); it is not committed and must not be hand-edited. `docs/User Guide/` should be edited via `pnpm edit-docs:edit-docs`, not manually. Only `docs/Developer Guide/` and `docs/Release Notes/` are safe for direct Markdown editing.
...
- Script API documentation generated by `apps/build-docs` into `site/script-api/` (published to docs.triliumnotes.org)
