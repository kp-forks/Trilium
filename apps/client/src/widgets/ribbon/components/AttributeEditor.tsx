...
import type {
    CommandData,
    FilteredCommandNames,
} from "../../../components/app_context";
import FAttribute from "../../../entities/fattribute";
import FNote from "../../../entities/fnote";
import contextMenu from "../../../menus/context_menu";
import attribute_parser, {
    Attribute,
} from "../../../services/attribute_parser";
import attribute_renderer from "../../../services/attribute_renderer";
import attributes from "../../../services/attributes";
import froca from "../../../services/froca";
import { t } from "../../../services/i18n";
import link from "../../../services/link";
import note_autocomplete, {
    Suggestion,
} from "../../../services/note_autocomplete";
...
const mentionSetup: MentionFeed[] = [
    {
        marker: "@",
        feed: (queryText) =>
            note_autocomplete.autocompleteSourceForCKEditor(queryText),
        itemRenderer: (_item) => {
    ...
    },
    {
        ...
        feed: async (queryText) => {
            const names = await server.get<string[]>(
                `attribute-names/?type=label&query=${encodeURIComponent(queryText)}`,
            );
    ...
    },
    {
        ...
        feed: async (queryText) => {
            const names = await server.get<string[]>(
                `attribute-names/?type=relation&query=${encodeURIComponent(queryText)}`,
            );
...
export default function AttributeEditor({
    ...
    const [state, setState] = useState<
        "normal" | "showHelpTooltip" | "showAttributeDetail"
    >();
    ...
    const [attributeDetailWidgetEl, attributeDetailWidget] = useLegacyWidget(
        () => new AttributeDetailWidget(),
    );
    ...
    async function renderOwnedAttributes(
        ownedAttributes: FAttribute[],
        saved: boolean,
    ) {
    ...
    function parseAttributes() {
        try {
            return attribute_parser.lexAndParse(
                getPreprocessedData(currentValueRef.current),
            );
        } catch (e: unknown) {
            setError(e);
        }
    ...
    useTriliumEvent("entitiesReloaded", ({ loadResults }) => {
        if (
            loadResults
                .getAttributeRows(componentId)
                .find((attr) => attributes.isAffecting(attr, note))
        ) {
            refresh();
        }
    });
    ...
    useLegacyImperativeHandlers(
        useMemo(
            () => ({
                loadReferenceLinkTitle: async (
                    $el: JQuery<HTMLElement>,
                    href: string,
                ) => {
                    const { noteId } = link.parseNavigationStateFromUrl(href);
                    const note = noteId
                        ? await froca.getNote(noteId, true)
                        : null;
                ...
                createNoteForReferenceLink: async (title: string) => {
                    let result;
                    if (notePath) {
                        result = await note_create.createNoteWithTypePrompt(
                            notePath,
                            {
                                activate: false,
                                title,
    ...
    useImperativeHandle(
        ...
        () => ({
            save,
            refresh,
            renderOwnedAttributes: (attributes) =>
                renderOwnedAttributes(attributes as FAttribute[], false),
                    ...
                    <div style="position: relative;">
                        <CKEditor
                            ...
                            onChange={(currentValue) => {
                                ...
                                const oldValue = getPreprocessedData(
                                    lastSavedContent.current ?? "",
                                ).trimEnd();
                                const newValue = getPreprocessedData(
                                    currentValue ?? "",
                                ).trimEnd();
                                ...
                                if (pos && pos.textNode && pos.textNode.data) {
                                    ...
                                    for (const attr of parsedAttrs) {
                                        if (
                                            ...
                                            clickIndex > attr.startIndex &&
                                            attr.endIndex !== undefined &&
                                            clickIndex <= attr.endIndex
                                        ) {
                                            matchedAttr = attr;
                                            break;
                                        }
                        ...
                        <div className="attribute-editor-buttons">
                            {needsSaving && (
                                <ActionButton
                                    ...
                                    text={escapeQuotes(
                                        t("attribute_editor.save_attributes"),
                                    )}
                                    onClick={save}
                                />
                            )}
                            ...
                            <ActionButton
                                ...
                                text={
                                    isMenuOpen
                                        ? ""
                                        : escapeQuotes(t("attribute_editor.add_a_new_attribute"))
                                }
                                ...
                                onClick={(e) => {
                                    ...
                                    contextMenu.show<AttributeCommandNames>({
                                        ...
                                        items: [
                                            {
                                                title: t(
                                                    "attribute_editor.add_new_label",
                                                ),
                                                command: "addNewLabel",
                                                uiIcon: "bx bx-hash",
                                            },
                                            {
                                                title: t(
                                                    "attribute_editor.add_new_relation",
                                                ),
                                                command: "addNewRelation",
                                                uiIcon: "bx bx-transfer",
                                            },
                                            { kind: "separator" },
                                            {
                                                title: t(
                                                    "attribute_editor.add_new_label_definition",
                                                ),
                                                command:
                                                    "addNewLabelDefinition",
                                                uiIcon: "bx bx-empty",
                                            },
                                            {
                                                title: t(
                                                    "attribute_editor.add_new_relation_definition",
                                                ),
                                                command:
                                                    "addNewRelationDefinition",
                                                uiIcon: "bx bx-empty",
                                            },
                                        ],
                                        selectMenuItemHandler: (item) =>
