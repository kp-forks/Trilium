...
/**
 * Maps a `KeyboardEvent.code` to a navigation direction. PageUp/PageDown (prev/next) are built in, as
 * are Home/End (first/last) unless `includeEdges` is false (e.g. media players reserve Home/End for
 * seeking); a renderer can supply extra codes for prev/next (e.g. the image viewer adds
 * `Backspace`/`Space`). Using `code` keeps it keyboard-layout independent.
 */
