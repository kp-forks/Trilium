...
function AnytypePanel({ parentNoteId, closeDialog, setFooter }: ImportProviderPanelProps) {
    ...
    const { hasSelection, displayNames, onChange, onBrowse, doImport } = useProviderImport({ format: "anytype", parentNoteId, shrinkImages, closeDialog });
        ...
        <Card heading={t("anytype_import.choose_file")}>
            <CardSection>
                <OptionsRow name="import-file" description={t("anytype_import.description_long")} stacked>
                    <FileDropZone onChange={onChange} onBrowse={onBrowse} displayNames={displayNames} accept=".zip" />
                </OptionsRow>
