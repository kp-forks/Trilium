/**
 * Renders preview.png — a faithful mock of the assembled DMG Finder window, so the
 * "what the DMG looks like" reference is reproducible from source rather than a mystery
 * committed image. Regenerate with: pnpm --filter desktop generate-dmg-preview
 *
 * This is documentation only: appdmg never sees it (it uses background.png/@2x).
 *
 * PIXEL FIDELITY. The icon positions here mirror the REAL DMG exactly, read back from a
 * built disk image's `.DS_Store`:
 *   - Finder `Iloc` records use a TOP-LEFT origin, y increasing downward, and (x, y) is
 *     the icon CENTER. (Confirmed by appdmg's own example: y=344 sits near the *bottom*.)
 *   - So the `contents` coordinates in forge.config.ts are top-left, NOT bottom-left.
 * The window chrome (title bar, traffic lights, title) is drawn by Finder/macOS, not by
 * appdmg — it is reconstructed here only so the reference reads as a real window.
 */
...
const APP = { x: 180, y: 200, label: "Trilium Notes" };
const APPS = { x: 460, y: 200, label: "Applications" };
...
// The trillium leaf mark (stable palette), reused from background.html for the title bar.
const TRILLIUM_MARK = `<svg viewBox="0 0 256 256" aria-hidden="true">
...
// Render the real macOS icons the DMG actually shows when generating on a Mac (the app's
// .icns squircle and the system Applications-folder icon). Off macOS `sips` is missing, so
// fall back to the flat logo / a drawn folder — the script still runs on any OS.
const work = mkdtempSync(path.join(tmpdir(), "dmg-preview-"));
const appIconHref =
    extractIcns(path.join(APP_ICON_DIR, "icon.icns"), work, "app.png") ??
    pathToFileURL(path.join(APP_ICON_DIR, "png", "128x128.png")).href;
...
function buildHtml() {
    ...
    .item .label {
        ...
        white-space: nowrap; font-size: 13px; font-weight: 500; color: #f4f5f7;
...
<div class="stage"><div class="win">
    <div class="titlebar">
        ...
        <span class="title">${TRILLIUM_MARK} Trilium Notes</span>
    </div>
    <div class="content">
        <img class="bg" src="${bgHref}">
        <div class="item" style="left:${APP.x}px;top:${APP.y}px">
            <img class="icon" src="${appIconHref}"><div class="label">${APP.label}</div>
        </div>
        <div class="item" style="left:${APPS.x}px;top:${APPS.y}px">
