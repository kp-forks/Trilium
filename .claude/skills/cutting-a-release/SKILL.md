...
- **Flathub does NOT publish itself.** `release.yml` has no Flathub job; `release-flathub.yml` is `workflow_dispatch`-only, so a release ships to Flathub only once someone runs `gh workflow run "Release to Flathub" -f ref=v<X.Y.Z>` and merges the pull request it opens against `flathub/org.triliumnotes.Trilium`. Needs the `FLATHUB_PAT` secret. See the `packaging-for-flathub` skill.
