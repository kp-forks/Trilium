...
description: >-
  ...
  a CKEditor 5 plugin for correctness, idiom, and project process: schema /
  conversion / command / UI / widget code, CKEditor-specific defects (asymmetric
  upcast/downcast, unconsumed upcast elements, missing inline-widget position
  mapping, command refresh/isEnabled bugs, memory leaks, t() gaps, editing/UI
  split violations, post-v42 import mistakes), and contribution process
  (changelog entries, commits, coverage). Pairs with the
  ckeditor5-plugin-development and ckeditor5-testing skills and delegates their
  checklists.
---
...
3. **Run the tests + coverage** for the affected package (see `ckeditor5-testing`). CKEditor holds
   **100% branch coverage**, but coverage ≠ correctness: confirm the *change itself* is tested,
   not just that lines are hit. A bug fix with no new/changed test is a red flag even at 100%.
...
8. **Check process** (if the project follows CKEditor's): changelog entry, commit/PR conventions,
   CLA, signed commits — `references/contribution-process.md`.
9. **Report** findings: severity-tagged, `file:line`, with the concrete fix or question.
...
| `references/contribution-process.md` | Project process a reviewer validates: file-based changelog entries (`.changelog/`, types/scope/closes), commit & PR conventions, branch naming, CLA, signed commits, coverage expectations. Relevant when reviewing for the ckeditor5 project or a package that adopts its process. |

## Provenance & source references

Self-contained and library-focused. Any `docs/…` / `packages/…` / `.changelog/` paths or the
baseline commit point into the **upstream CKEditor 5 repository**
([github.com/ckeditor/ckeditor5](https://github.com/ckeditor/ckeditor5)), not the project you are
reviewing. Distilled at upstream commit **`9ecca53627`** (mid-2026) from
`docs/framework/contributing/{contributing,changelog-entries,code-style}.md` and the patterns in
the companion skills. To refresh, compare those paths against that commit inside a clone of
ckeditor5 and bump the reference here.
