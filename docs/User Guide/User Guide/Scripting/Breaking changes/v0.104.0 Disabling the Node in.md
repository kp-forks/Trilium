...
The `shell` group is the most heavily validated: `openExternal` is restricted to an allowlisted set of URL schemes (blocking `file:`, `smb:`, Follina-class schemes, etc.), `openPath` / `openFileUrl` must resolve under the Trilium data directory or temporary directory, and `downloadURL` is locked to the app's own origin. Invalid input throws on the main side rather than being silently passed through.