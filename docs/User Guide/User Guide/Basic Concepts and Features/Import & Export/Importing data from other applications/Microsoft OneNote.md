...
*   Tags (apart from to-do lists) are preserved by converting them to inline icons, which are searchable by the icon's name and can be recoloured like any other icon. The Project A/B and numbered priority tags keep an emoji instead, as the icon pack has no letters or numerals.
